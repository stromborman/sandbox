...
Two Sum:
    ...
    return distinct indices such that the numbers add up to target.
    Done in O(n) via hash
"""

def twoSum(nums: List[int], target: int) -> List[int]:
    hash = {y:j for j,y in enumerate(nums)}
    for i,x in enumerate(nums):
        # Just a 'faster' (?) way of doing (target-x) in hash.keys()
        try:
            j = hash[target - x]
            if i !=j:
                return [i,j] # assuming there is only one solution
        except:
            pass
